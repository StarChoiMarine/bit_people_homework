...
직원 초기 비밀번호는 한글 이름을 영문 키보드 자판으로 입력한 문자열 뒤에 `@@`를 붙인
데모 규칙을 사용했습니다. 예를 들어 김민준은 `rlaalswns@@`입니다. 새 계정을 만들거나
비밀번호를 변경할 때는 8자 이상이며 특수문자를 포함해야 합니다.

> 이 자격증명은 평가와 기능 확인만을 위한 공개된 초기값입니다. 실제 운영 배포 전에는
> 모든 초기 비밀번호를 교체하고, 별도의 안전한 전달 및 최초 로그인 변경 정책을 적용해야
> 합니다. `admin123`도 운영 환경에서는 즉시 변경해야 합니다.
...
또한 실제 운영 전에는 공개된 시드 비밀번호 제거, HTTPS와 Secure 쿠키 강제, CSRF 보호,
